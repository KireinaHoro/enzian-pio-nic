...
#define PIONIC_ECI_INLINE_DATA_SIZE                                            \
  (PIONIC_ECI_CL_SIZE - PIONIC_ECI_INLINE_DATA_OFFSET)
#define PIONIC_ECI_WORKER_CTRL_INFO_BASE                                       \
  (0xFF80) // currently last CL in the region PIONIC_ECI_CORE_OFFSET

// mapped pages layout:
//
// PIONIC_ECI_RX_BASE                                   ECI host control info
// for RX - host_ctrl_info_*_t
//                    + PIONIC_ECI_INLINE_DATA_OFFSET   RX inline data
// PIONIC_ECI_RX_BASE + PIONIC_ECI_CL_SIZE              the other RX CL
// PIONIC_ECI_RX_BASE + PIONIC_ECI_OVERFLOW_OFFSET      RX overflow cachelines
// (count: PIONIC_ECI_NUM_OVERFLOW_CL)
//
// PIONIC_ECI_TX_BASE                                   ECI host control info
// for TX - host_ctrl_info_*_t
//                    + PIONIC_ECI_INLINE_DATA_OFFSET   RX inline data
// PIONIC_ECI_TX_BASE + PIONIC_ECI_CL_SIZE              the other TX CL
// PIONIC_ECI_TX_BASE + PIONIC_ECI_OVERFLOW_OFFSET      TX overflow cachelines
// (count: PIONIC_ECI_NUM_OVERFLOW_CL)
//
...
//
STATIC_ASSERT(PIONIC_ECI_OVERFLOW_OFFSET >= PIONIC_ECI_CL_SIZE * 2,
              "overlapped");
STATIC_ASSERT(PIONIC_ECI_TX_BASE >=
                  PIONIC_ECI_OVERFLOW_OFFSET +
                      PIONIC_ECI_NUM_OVERFLOW_CL * PIONIC_ECI_CL_SIZE,
              "overlapped");
STATIC_ASSERT(PIONIC_ECI_WORKER_CTRL_INFO_BASE >=
                  PIONIC_ECI_TX_BASE + PIONIC_ECI_OVERFLOW_OFFSET +
                      PIONIC_ECI_NUM_OVERFLOW_CL * PIONIC_ECI_CL_SIZE,
              "overlapped");
STATIC_ASSERT(PIONIC_ECI_CORE_OFFSET >=
                  PIONIC_ECI_WORKER_CTRL_INFO_BASE + PIONIC_ECI_CL_SIZE,
              "overlapped");
...
static bool core_eci_rx(void *base, volatile bool *rx_parity_ptr,
  ...
  uint8_t *worker_ctrl_addr =
      (uint8_t *)base + PIONIC_ECI_WORKER_CTRL_INFO_BASE;
  ...
  while (!COMPARE_AND_SWAP(worker_ctrl_addr, (uint8_t)0b01, (uint8_t)0b11))
    BARRIER // make sure BUSY actually took effect
        critical_section_start : pr_debug("eci_rx: entered critical section\n");

  bool rx_parity = *rx_parity_ptr;
  pr_debug("eci_rx: current cacheline ID: %d\n", rx_parity);

  uint8_t *rx_base =
      (uint8_t *)base + (PIONIC_ECI_RX_BASE + rx_parity * PIONIC_ECI_CL_SIZE);
  ...
  if (!valid) {
    ...
    case pionic_eci_bypass:
      ...
      memcpy(desc->bypass.header,
             rx_base + pionic_eci_host_ctrl_info_bypass_size,
    ...
    case pionic_eci_onc_rpc_call:
      ...
      memcpy(desc->oncrpc_call.args,
             rx_base + pionic_eci_host_ctrl_info_onc_rpc_call_size,
    ...
    if (pkt_len == 0) {
      ...
      int first_read_size = pkt_len > PIONIC_ECI_INLINE_DATA_SIZE
                                ? PIONIC_ECI_INLINE_DATA_SIZE
                                : pkt_len;
      memcpy(desc->payload_buf, rx_base + PIONIC_ECI_INLINE_DATA_OFFSET,
             first_read_size);
      if (pkt_len > PIONIC_ECI_INLINE_DATA_SIZE) {
        memcpy(desc->payload_buf + PIONIC_ECI_INLINE_DATA_SIZE,
               rx_base + PIONIC_ECI_OVERFLOW_OFFSET,
               pkt_len - PIONIC_ECI_INLINE_DATA_SIZE);
      }
    }
...
static void core_eci_tx(void *base, volatile bool *tx_parity_ptr,
                        pionic_pkt_desc_t *desc) {
  ...
  while (!COMPARE_AND_SWAP((uint8_t *)worker_ctrl_addr, (uint8_t)0b01,
                           (uint8_t)0b11))
    BARRIER // make sure BUSY actually took effect
        critical_section_start : pr_debug("eci_tx: entered critical section\n");

  bool tx_parity = *tx_parity_ptr;
  pr_debug("eci_tx: current cacheline ID: %d\n", tx_parity);

  uint8_t *tx_base =
      (uint8_t *)base + PIONIC_ECI_TX_BASE + tx_parity * PIONIC_ECI_CL_SIZE;
  ...
  case TY_BYPASS:
    ...
    case HDR_ETHERNET:
      pionic_eci_host_ctrl_info_bypass_hdr_ty_insert(tx_base,
                                                     pionic_eci_hdr_ethernet);
      break;
    case HDR_IP:
      pionic_eci_host_ctrl_info_bypass_hdr_ty_insert(tx_base,
                                                     pionic_eci_hdr_ip);
    ...
    case HDR_ONCRPC_CALL:
      pionic_eci_host_ctrl_info_bypass_hdr_ty_insert(
          tx_base, pionic_eci_hdr_onc_rpc_call);
      break;
    }
    memcpy(tx_base + pionic_eci_host_ctrl_info_bypass_size, desc->bypass.header,
           sizeof(desc->bypass.header));
    break;

  case TY_ONCRPC_CALL:
    pionic_eci_host_ctrl_info_error_ty_insert(tx_base,
                                              pionic_eci_onc_rpc_reply);
    ...
    memcpy(tx_base + pionic_eci_host_ctrl_info_onc_rpc_call_size,
           desc->oncrpc_call.args, sizeof(desc->oncrpc_call.args));
    break;

  case TY_ONCRPC_REPLY:
    pionic_eci_host_ctrl_info_onc_rpc_reply_ty_insert(tx_base,
                                                      pionic_eci_onc_rpc_reply);
    memcpy(tx_base + pionic_eci_host_ctrl_info_onc_rpc_reply_size,
  ...
  if (desc->payload_buf != NULL) {
    ...
    memcpy((void *)(tx_base + PIONIC_ECI_INLINE_DATA_OFFSET), desc->payload_buf,
           first_read_size);
    if (desc->payload_len > PIONIC_ECI_INLINE_DATA_SIZE) {
      // XXX: user can overflow the overflow CLs, better give an error
      memcpy(tx_base + PIONIC_ECI_OVERFLOW_OFFSET,
             desc->payload_buf + PIONIC_ECI_INLINE_DATA_SIZE,
             desc->payload_len - PIONIC_ECI_INLINE_DATA_SIZE);
    }
  }
  ...
  (void)*((volatile uint8_t *)base +
          (PIONIC_ECI_TX_BASE + !tx_parity * PIONIC_ECI_CL_SIZE));
