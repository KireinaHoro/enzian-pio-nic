...
void pionic_read_timestamps_core_ctrl(pionic_global_t *dev, pionic_core_ctrl_timestamps_t *ts) {
  ...
  ts->rx_after_cdc_queue = pionic_global(last_profile__rx_after_cdc_queue_rd)(dev);
  ts->rx_enqueue_to_host = pionic_global(last_profile__rx_enqueue_to_host_rd)(dev);
  ts->rx_core_read_start = pionic_global(last_profile__rx_core_read_start_rd)(dev);
  ts->rx_core_read_finish = pionic_global(last_profile__rx_core_read_finish_rd)(dev);
  ...
  ts->tx_after_dma_read = pionic_global(last_profile__tx_after_dma_read_rd)(dev);
  ts->tx_before_cdc_queue = pionic_global(last_profile__tx_before_cdc_queue_rd)(dev);
