...
/**
  ...
  *
  * Encoder stages form a DAG, analogous to [[Decoder]].  The DAG is specified by [[to]].
  *
...
trait Encoder[T <: EncoderMetadata] extends FiberPlugin {
  ...
  val txRg = during setup retains(host[EncoderSource].retainer)
  ...
  /**
    ...
    *  - packets from the host, passed through [[EncoderSource]]
    *
  ...
  protected def collectInto(metadata: Stream[T], payload: Axi4Stream, acceptHostPackets: Boolean = false): Unit = new Composite(this, "consume") {
    ...
    if (acceptHostPackets) {
      ...
      host[EncoderSource].connect(ty, hostDesc, hostPayload)
