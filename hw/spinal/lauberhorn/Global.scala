...
object Global extends AreaRoot {
  ...
  val ECI_CORE_OFFSET = blocking[Long]
  ...
  def writeConfigs(outPath: os.Path, spinalConfig: SpinalConfig): Unit = {
    ...
    os.write(outPath,
      f"""|#ifndef __LAUBERHORN_CONFIG_H__
          ...
          |
          |#define LAUBERHORN_CLOCK_FREQ ${spinalConfig.defaultClockDomainFrequency.getValue.toLong}
          |
            ${
        vals.map { case (k, v) =>
          s"|#define LAUBERHORN_$k ($v)"
        }.mkString("\n")
      }
          |
  ...
  def initDatabase(nc: Int, nw: Int) = {
    ...
    val bufSizeMap = Seq(
      128  -> .2,
      1518 -> .8, // max Ethernet frame with MTU 1500
    )
    ...
    PKT_BUF_RX_SIZE_PER_CORE.set(16 * 1024)
