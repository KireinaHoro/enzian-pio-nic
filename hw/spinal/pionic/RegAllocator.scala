    ...
    val defLines = blocks.flatMap { case (blockName, block) =>
      ...
      if (block.allocatedBases.size == 1) {
        block.blockMap.flatMap { case (name, desc) =>
          ...
          Seq(
            f"#define PIONIC_${bname}_$rname ${desc.addr(base)}%#x",
            f"#define PIONIC_${bname}_${rname}_SIZE ${desc.size}%#x",
          )
        }
      } else {
        val arrayName = s"__${blockName.toCName}_bases"
        Seq(
          ...
          val rname = name.toCMacroName
          Seq(
            f"#define PIONIC_${bname}_$rname(blockIdx) (${desc.baseOffset}%#x + $arrayName[blockIdx])",
            f"#define PIONIC_${bname}_${rname}_SIZE ${desc.size}%#x",
          )
        }
      } :+ ""
