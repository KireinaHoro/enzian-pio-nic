...
/** ID of a packet in the packet-processing pipeline.
  ...
  * - TX HostMsgID: generated by the ECI TX host interface and passed as
  *   trace-only sideband to DmaControlPlugin.  It is emitted on ECI TX and DMA
  *   TX events without changing host-visible CL contents.  Correlation to RpcID
  *   is left to the host-message-to-RPC handoff.
  */
...
case object HostReqTy     extends TraceDataKey { def width = Global.HOST_REQ_TY_WIDTH.get }
case object PacketDescTy  extends TraceDataKey { def width = Global.PKT_DESC_TY_WIDTH.get }
case object LogicalLength extends TraceDataKey { def width = Global.PKT_BUF_LEN_WIDTH.get }
case object DmaLength     extends TraceDataKey { def width = Global.PKT_BUF_LEN_WIDTH.get }
