...
def sample_lost_count(sample: int, trace_map: Dict[str, Any]) -> int:
    lost_count_width = int(trace_map["sample"].get("lost_count_width", 32))
    return bits(sample_payload(sample, trace_map), 0, lost_count_width)


def sample_is_bubble_marker(sample: int, trace_map: Dict[str, Any]) -> bool:
    sample_cfg = trace_map["sample"]
    lost_source = int(sample_cfg.get("lost_source", (1 << int(sample_cfg["source_width"])) - 1))
    return sample_source(sample, trace_map) == lost_source and sample_lost_count(sample, trace_map) == 0
...
def scan_samples(
    ...
    Bubble markers are generated by the packer at flush time and may have a
    newer timestamp than the real samples they flush, so they are ignored for
    timestamp-order wrap detection. A circular-buffer boundary appears as a
    large modulo jump from the newest real/control sample back to older samples.
    ...
    for index, sample in iter_sample_range(
        ...
        if not sample_is_bubble_marker(sample, trace_map):
            if previous_ts is not None:
                delta = (ts - previous_ts) % modulus
                if ts < previous_ts and delta > modulus // 2:
                    print(
                        "warning: real trace sample timestamp decreased "
                        f"at sample {index}: previous sample {previous_index} "
                        f"timestamp {previous_ts}, current timestamp {ts}",
                        file=sys.stderr,
                    )
                if delta > max_delta:
                    max_delta = delta
