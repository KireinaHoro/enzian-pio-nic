...
| Package generated RTL | `nix build .#genVerilog -L` | `result` contains RTL/XDC; derivation also has `headers` and `devices` outputs. Current package install omits trace-map JSON. |
...
`pcie.generateVerilog` and `pcie.generateBitstream` remain legacy targets; PCIe hardware CI is commented out. ECI bitstream task return paths in `build.mill` still describe the ordinary Vivado `impl_1/lauberhorn-eci.bit` layout, while the sourced static-shell Tcl emits the combined shell filenames above. Check this discrepancy when diagnosing task completion/caching; documentation cannot establish that the current task succeeds.
...
- For trace acquisition/export, use [trace tool instructions](../../data/eci/sys_trace/README.md) and [trace architecture](tracing.md). Pair dumps with the trace-map JSON from the matching FPGA build. The Nix `genVerilog` default output currently does not retain this map.

## CI and documentation drift

`.gitlab-ci.yml` is GitLab CI using the `spinal-verilator:ubuntu-22.04` container, rather than the local Nix shell. `fast-tests-eci` runs fast ECI tests; `blocks-tests` is conditional on submodule changes; `build-hw-eci` depends on fast ECI tests; `publish` runs for tags.

Known gaps to check before relying on release automation:

- CI header/device artifact globs still use `hw/gen/*/*.h` and `sw/devices/lauberhorn_*.dev`; the generator now writes these under `out/eci/generateVerilog.dest/`.
- Release paths still include old `sw/{eci,pcie}/pionic-test` binaries and disabled PCIe build artifacts.
- There are no software-build, full-slow-suite, or TLA+ jobs in this CI file.
- `sw/README.md` still names obsolete `.#lauberhorn-kmod`/`.#lauberhorn-rt` packages and inconsistent shared-library names. Use the package table above.
